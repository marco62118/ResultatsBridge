package embrun.fr.tournoibridgeonline.client
...
import embrun.fr.tournoibridgeonline.client.screens.VerificationDonneEcran
import embrun.fr.tournoibridgeonline.client.screens.VerificationMainsScreen
import embrun.fr.tournoibridgeonline.common.AppScaffold
import embrun.fr.tournoibridgeonline.common.model.*
import embrun.fr.tournoibridgeonline.common.theme.BridgeServeurTheme
import embrun.fr.tournoibridgeonline.screens.*
import embrun.fr.tournoibridgeonline.organisateur.data.DatabaseManager
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext
import java.text.SimpleDateFormat
import java.util.*
import embrun.fr.tournoibridgeonline.common.model.TournoiConfig
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            BridgeServeurTheme {
                ...
                val changementDeMouvement  = remember { mutableStateOf<ChangementDeMouvement?>(null) }
                ...
                var listeDetailsDonnes  by remember { mutableStateOf<List<DonneResultatDetail>>(emptyList()) }
                ...
                val afficherErreur: (String, EtapeClient) -> Unit = { message, retour ->
                    ...
                    coroutineScope.launch(Dispatchers.IO) {
                        if (useLocal) DatabaseManager.enregistrerErreurLog(context, tournoiId, equipeNum, etapeNom, message)
                        else ClientNetworkUtils.logErreurServeur(tournoiId, equipeNum, etapeNom, message)
                    }
                }

                // ── États vérification mains ──────────────────────────────────────
                var numeroDonneAVerifier    by remember { mutableStateOf(0) }
                var equipesAyantJoueDonne   by remember { mutableStateOf<List<EquipeDonneInfo>>(emptyList()) }
                    ...
                    if (OrganisateurIsJoueur && idTournoiExtra != -1) {
                        if (useLocal) {
                            withContext(Dispatchers.IO) {
                                val (idOuvert, typeTournoi) = DatabaseManager.getTournoiOuvert(context) ?: (null to null)
                                if (idOuvert != null && idOuvert == idTournoiExtra && typeTournoi != null) {
                                    tournoi.value = idOuvert to typeTournoi
                                    val liste = DatabaseManager.getEquipesDuTournoi(context, idTournoiExtra)
                                    if (liste.isNotEmpty()) {
                                        equipes.addAll(liste)
                                    } else {
                                        tournoi.value = null
                                        afficherErreur("Aucune équipe trouvée pour ce tournoi.", EtapeClient.CONNEXION)
                                    }
                                } else {
                                    tournoi.value = null
                                    afficherErreur("Impossible de charger le tournoi.", EtapeClient.CONNEXION)
                                }
                            }
                        } else {
                            withContext(Dispatchers.IO) {
                                ...
                                if (tournoiOnline != null) {
                                    ...
                                    val liste = ClientNetworkUtils.recupererListeEquipes(tournoiOnline.first)
                                    if (!liste.isNullOrEmpty()) {
                                        equipes.addAll(liste)
                                    } else {
                                        afficherErreur("Aucune équipe trouvée sur le serveur.", EtapeClient.CONNEXION)
                                    }
                                } else {
                                    afficherErreur("Aucun tournoi ouvert sur le serveur.", EtapeClient.CONNEXION)
                                }
                            }
                        }
                    }
                ...
                if (etape.value == EtapeClient.AFFICHAGE_MAINS) {
                    ...
                    val mainsValides = mainsDirect != null && mainsDirect.size == 4 && mainsDirect.all { it.size == 13 }
                    if (mainsValides) {
                        ...
                        AffichageMainsScreen(
                            numeroDonne = numDonne,
                            onRetour = { aCliqueAfficherMains.value = false; etape.value = EtapeClient.MOUVEMENT_EN_COURS },
                            onEnregistrer = { mainsSelectionnees.value = it; etape.value = EtapeClient.MOUVEMENT_EN_COURS }
                        )
                    }
                    ...
                    if (mainsValides) {
                        AffichageMainsLectureScreen(
                            numeroDonne = numDonne,
                            mains = mainsExistantes!!.map { codes -> codes.map { Carte.fromCode(it) } },
                        ...
                        AffichageMainsScreen(
                            ...
                            onEnregistrer = { mainsCartes ->
                                val mainsCodesString = mainsCartes.map { main -> main.map { it.code } }
                                val idxSaisi = indexDonneRelaisEnCours
                                coroutineScope.launch {
                                    val ok = withContext(Dispatchers.IO) {
                                        if (useLocal) DatabaseManager.enregistrerMainsRelais(context, idTournoi, numDonne, mainsCodesString)
                                        else ClientNetworkUtils.enregistrerMainsRelais(idTournoi, numDonne, mainsCodesString)
                                    }
                                    if (ok) {
                                        ...
                                        nombreDonnesRelaisSaisies = mouvementRelais?.donnes?.count { d ->
...
*/
                    ...
                    AffichageDonneEcrant(
                        idTournoi = idTournoi, dateTournoi = dateTournoi,
                        numeroDonne = numDonneSelect, equipeNS = eqNSSelect, equipeEO = eqEOSelect,
                        mains = mainsAAfficher ?: emptyMap(), encheres = encheresAAfficher,
                        vulnerable = vulnerable, donneur = donneur, contrat = contrat, declarant = declarant,
                    ...
                    VerificationDonneEcran(
                        numeroDonne = numDonneSelect, mains = mainsAAfficher!!,
                        equipeNS = eqNSSelect, equipeEO = eqEOSelect,
                        vulnerable = vulnerable, donneur = donneur, contrat = contrat, declarant = declarant,
                    ...
                    AppScaffold(ipAddress = ipServeurExtra, numeroTournoi = tournoi.value?.first, equipeChoisie = equipeChoisie.value) { }

                    androidx.compose.material3.AlertDialog(
                        onDismissRequest = { },
                        title = {
                            Text("🏖️ Table Relais - Mouvement ${mvntTermineNumero.value ?: "?"}", style = MaterialTheme.typography.headlineMedium)
                        },
                        text = {
                            Column(modifier = Modifier.fillMaxWidth().heightIn(max = 500.dp).verticalScroll(rememberScrollState())) {
                                Text("Installez-vous à la table relais.\n\nPas de donnes à jouer ! Vous pouvez saisir les mains pour aider les autres équipes.", style = MaterialTheme.typography.bodyLarge)
                                Spacer(Modifier.height(12.dp))

                                mouvementRelais?.donnes?.forEachIndexed { idx, donne ->
                                    val mainsDejaLa = donne.mains != null && donne.mains.size == 4 && donne.mains.all { it.size == 13 }
                                    OutlinedButton(
                                        onClick = { indexDonneRelaisEnCours = idx; etape.value = EtapeClient.SAISIE_MAINS_RELAIS },
                                        modifier = Modifier.fillMaxWidth().padding(vertical = 3.dp),
                                        colors = ButtonDefaults.outlinedButtonColors(contentColor = if (mainsDejaLa) Color(0xFF2E7D32) else MaterialTheme.colorScheme.primary),
                                        border = BorderStroke(1.dp, if (mainsDejaLa) Color(0xFF2E7D32) else MaterialTheme.colorScheme.primary)
                                    ) {
                                        Text(if (mainsDejaLa) "👁 Voir mains - Donne ${donne.numero}" else "📤 Saisir mains - Donne ${donne.numero}", fontWeight = if (mainsDejaLa) FontWeight.Bold else FontWeight.Normal)
                                    }
                                }

                                Spacer(Modifier.height(12.dp))

                                if ((mvntTermineNumero.value ?: 0) < TournoiConfig.NBRE_MOUVEMENTS) {
                                    if (changementDeMouvement.value != null) {
                                        Text("➡️ Votre prochain mouvement :", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
                                        Spacer(Modifier.height(8.dp))
                                        changementDeMouvement.value?.entries?.forEach { entry ->
                                            Card(modifier = Modifier.fillMaxWidth().padding(vertical = 4.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.primaryContainer)) {
                                                Column(modifier = Modifier.padding(12.dp)) {
                                                    Text("Mouvement ${changementDeMouvement.value?.mvntSuivant ?: "?"} - Table ${entry.tableNumero}", style = MaterialTheme.typography.titleSmall, fontWeight = FontWeight.Bold)
                                                    Spacer(Modifier.height(4.dp))
                                                    Text("Éq. ${entry.equipe.equipeNumero} (NS) : ${entry.equipe.joueur1.prenom} ${entry.equipe.joueur1.nom} & ${entry.equipe.joueur2.prenom} ${entry.equipe.joueur2.nom}", style = MaterialTheme.typography.bodySmall)
                                                    Text("Éq. ${entry.adversaire.equipeNumero} (EO) : ${entry.adversaire.joueur1.prenom} ${entry.adversaire.joueur1.nom} & ${entry.adversaire.joueur2.prenom} ${entry.adversaire.joueur2.nom}", style = MaterialTheme.typography.bodySmall)
                                                }
                                            }
                                        }
                                    } else {
                                ...
                                if (!tableSuivanteVisible && totalDonnes > 0) {
                                    Spacer(Modifier.height(8.dp))
                                    Text("💡 Saisissez les $totalDonnes donnes pour débloquer 'Table Suivante', ou appuyez sur 'Passer'.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                                }
                            }
                        },
                        confirmButton = {
                            val actionSuivante = {
                                ...
                                if (mvntCourant >= TournoiConfig.NBRE_MOUVEMENTS) etape.value = EtapeClient.ATTENTE_CLASSEMENT
                        ...
                        when (etape.value) {
                            ...
                            EtapeClient.SELECTION_EQUIPE -> {
                                ...
                                if (currentTournoi != null) {
                                    SelectionEquipeScreen(
                                        ...
                                        equipes = equipes,
                                        onEquipeChoisie = { equipeChoisie.value = it; etape.value = EtapeClient.RECUPER_MOUVEMENT }
                                    )
                                } else {
                                    EcranMessage(titre = "Chargement en cours...", message = "Veuillez patienter.", texteBouton = "Réessayer", onOk = { etape.value = EtapeClient.CONNEXION })
                                }
                            }
                            ...
                            EtapeClient.RECUPER_MOUVEMENT -> {
                                ...
                                LaunchedEffect(equipeChoisie.value) {
                                    ...
                                    Log.i("RECUPER_MOUVEMENT", "🔁 #$num — équipe=${eq.equipeNumero}, tournoiId=$tournoiId")

                                    val result = withContext(Dispatchers.IO) {
                                        if (useLocal) DatabaseManager.getMouvementPourEquipe(context, tournoiId, eq.equipeNumero)
                                        else ClientNetworkUtils.recupererMouvement(tournoiId, eq.equipeNumero)
                                    }
                                    Log.i("RECUPER_MOUVEMENT", "📦 #$num → résultat=${result?.javaClass?.simpleName ?: "null"}")

                                    when (result) {
                                        is MouvementResult.Complet -> {
                                            val mvnt = result.mouvement
                                            val estRelais = listOf(mvnt.joueur1NSNom, mvnt.joueur2NSNom, mvnt.joueur1EONom, mvnt.joueur2EONom)
                                                .any { it.trim().lowercase() == "relais" }
                                            Log.i("RECUPER_MOUVEMENT", "🃏 #$num — mvnt=${mvnt.mvntNumero}, table=${mvnt.tableNumero}, relais=$estRelais")
                                            ...
                                            if (mvnt.indexDonneAJouer == 0 && mvnt.mvntNumero > 1 && !result.tousTermines) {
                                                ...
                                                contratFinal.value = null; mainsSelectionnees.value = null; aCliqueAfficherMains.value = false
                                                mvntTermineNumero.value  = mvnt.mvntNumero - 1
                                                equipesMvntTermine.value = mvnt.equipeNS to mvnt.equipeEO
                                                Log.i("RECUPER_MOUVEMENT", "✅ #$num → CHANGEMENT_DE_MOUVEMENT (mvntTermine=${mvnt.mvntNumero - 1})")
                                                ...
                                                if (estRelais) {
                                                    ...
                                                    indexDonneRelaisEnCours = 0
                                                    nombreDonnesRelaisSaisies = mvnt.donnes.count { d ->
                                                        d.mains != null && d.mains.size == 4 && d.mains.all { it.size == 13 }
                                                    }
                                                    mvntTermineNumero.value  = mvnt.mvntNumero
                                                    equipesMvntTermine.value = mvnt.equipeNS to mvnt.equipeEO
                                                    Log.i("RECUPER_MOUVEMENT", "⏩ #$num — passerTableRelais équipe=${eq.equipeNumero}")
                                                    withContext(Dispatchers.IO) {
                                                        if (useLocal) DatabaseManager.incrementerMouvementEquipe(context, tournoiId, eq.equipeNumero)
                                                        else ClientNetworkUtils.passerTableRelais(tournoiId, eq.equipeNumero)
                                                    }
                                                    val futurResult = withContext(Dispatchers.IO) {
                                                        if (useLocal) DatabaseManager.getMouvementPourEquipe(context, tournoiId, eq.equipeNumero)
                                                        else ClientNetworkUtils.recupererMouvement(tournoiId, eq.equipeNumero)
                                                    }
                                                    val futurMvnt = (futurResult as? MouvementResult.Complet)?.mouvement
                                                    Log.i("RECUPER_MOUVEMENT", "🔮 #$num — futurMvnt=${futurMvnt?.mvntNumero}, table=${futurMvnt?.tableNumero}")
                                                    if (futurMvnt != null) {
                                                        mouvement.value = futurMvnt
                                                        indexDonneAJouer.value = futurMvnt.indexDonneAJouer
                                                        contratFinal.value = null; mainsSelectionnees.value = null; aCliqueAfficherMains.value = false
                                                    }
                                                    changementDeMouvement.value = futurMvnt?.let { f ->
                                                        ChangementDeMouvement(
                                                            mvntSuivant = f.mvntNumero,
                                                            entries = listOf(ChangementDeMouvementEntry(
                                                                tableNumero = f.tableNumero,
                                                                equipe      = Equipe(f.equipeNS, Joueur(0, f.joueur1NSNom, f.joueur1NSPrenom), Joueur(0, f.joueur2NSNom, f.joueur2NSPrenom), tournoiId),
                                                                adversaire  = Equipe(f.equipeEO, Joueur(0, f.joueur1EONom, f.joueur1EOPrenom), Joueur(0, f.joueur2EONom, f.joueur2EOPrenom), tournoiId)
                                                            ))
                                                        )
                                                    }
                                                    Log.i("RECUPER_MOUVEMENT", "✅ #$num → DIALOG_RELAIS")
                                                    etape.value = EtapeClient.DIALOG_RELAIS
                                                    ...
                                                    Log.i("RECUPER_MOUVEMENT", "✅ #$num → MOUVEMENT_EN_COURS (indexDonne=${mvnt.indexDonneAJouer})")
                                                    etape.value = EtapeClient.MOUVEMENT_EN_COURS
                                                }
                                            }
                                        }
                                        is MouvementResult.ClassementEnAttente -> {
                                            Log.i("RECUPER_MOUVEMENT", "✅ #$num → ATTENTE_CLASSEMENT")
                                            etape.value = EtapeClient.ATTENTE_CLASSEMENT
                                        }
                                        is MouvementResult.Erreur -> {
                                            Log.e("RECUPER_MOUVEMENT", "❌ #$num → Erreur: ${result.message}")
                                            afficherErreur("Impossible de récupérer le mouvement.", EtapeClient.RECUPER_MOUVEMENT)
                                        }
                                        else -> {
                                            Log.e("RECUPER_MOUVEMENT", "❌ #$num → Résultat inattendu: ${result?.javaClass?.simpleName ?: "null"}")
                                            afficherErreur("Erreur inattendue lors du chargement.", EtapeClient.RECUPER_MOUVEMENT)
                                        }
                                    }
                                }
                            }
                            EtapeClient.MOUVEMENT_EN_COURS -> {
                                ...
                                MouvementScreen(
                                    modeJeu = modeJeu, equipe = eq, date = dateTournoi,
                                    idTournoi = tournoiId, mouvement = mvnt, contratFinal = contratFinal.value,
                                    typeTournoi = tournoi.value?.second ?: "",
                                    onEditEncheres = {
                                        donneur = mouvement.value?.donnes?.getOrNull(mouvement.value?.indexDonneAJouer ?: 0)?.donneur ?: "N"
                                    ...
                                    onAfficherMains  = { aCliqueAfficherMains.value = true; visiteMainsEffectuee.value = true; etape.value = EtapeClient.AFFICHAGE_MAINS },
                                    onDonneSuivante  = { },
                                    onEnregistrerDonne = { donne ->
                                        donneAEnregistrer.value = donne.copy(mains = mainsSelectionnees.value?.map { m -> m.map { it.code } })
                                    ...
                                    onVerifierMains = { numeroDonne ->
                                        ...
                                        coroutineScope.launch {
                                            chargementEquipesEnCours = true; erreurChargementEquipes = null
                                            try {
                                                equipesAyantJoueDonne = if (useLocal)
                                                    withContext(Dispatchers.IO) { DatabaseManager.getEquipesAyantJoueDonne(context, idTournoi, numeroDonne) }
                                                else ClientNetworkUtils.getEquipesAyantJoueDonne(idTournoi, numeroDonne)
                                                if (equipesAyantJoueDonne.isEmpty()) erreurChargementEquipes = "Aucune équipe n'a encore joué cette donne"
                                            } catch (e: Exception) { erreurChargementEquipes = "Erreur de chargement" }
                                            finally { chargementEquipesEnCours = false }
                            ...
                            EtapeClient.VERIFICATION_MAINS -> {
                                VerificationMainsScreen(
                                    idTournoi = idTournoi, numeroDonne = numeroDonneAVerifier,
                                    equipesAyantJoue = equipesAyantJoueDonne,
                                    isLoading = chargementEquipesEnCours, errorMessage = erreurChargementEquipes,
                                    onBack = { etape.value = EtapeClient.MOUVEMENT_EN_COURS },
                                    onAfficherDonne = {
                                        coroutineScope.launch {
                                            try {
                                                val dc = if (useLocal)
                                                    withContext(Dispatchers.IO) { DatabaseManager.getDonneComplete(context, idTournoi, numeroDonneAVerifier, equipesAyantJoueDonne.first().equipeNS) }
                                                else ClientNetworkUtils.recupererDonneComplete(idTournoi, numeroDonneAVerifier, equipesAyantJoueDonne.first().equipeNS)
                                                if (dc != null) {
                                                    mainsAAfficher = dc.mains; encheresAAfficher = dc.encheres
                                                    vulnerable = dc.vulnerable; donneur = dc.donneur
                                                    contrat = dc.contrat; declarant = dc.declarant
                                                    numDonneSelect = numeroDonneAVerifier
                                                    eqNSSelect = equipesAyantJoueDonne.first().equipeNS
                                                    eqEOSelect = equipesAyantJoueDonne.first().equipeEO
                                                    etape.value = EtapeClient.VERIFICATION_MAINS_AFFICHAGE
                                                }
                                            } catch (e: Exception) {
                                Log.e("ClientActivity", "❌ Erreur chargement mains : ${e.message}")
                                afficherErreur("Impossible d'afficher les mains.", EtapeClient.VERIFICATION_MAINS)
                            }
                                        }
                                    }
                                )
                            ...
                            EtapeClient.ENCHERES -> {
                                val mvnt  = mouvement.value ?: return@AppScaffold
                                val donne = mvnt.donnes?.getOrNull(mvnt.indexDonneAJouer) ?: return@AppScaffold
                                ...
                                LaunchedEffect(donne) {
                                    ...
                                    val retour = withContext(Dispatchers.IO) {
                                        if (useLocal) DatabaseManager.enregistreDonne(
                                            context, idTournoi = tournoi.value!!.first,
                                            mvntNumero = donne.mvntNumero, equipeNS = donne.equipeNS!!, equipeEO = donne.equipeEO!!,
                                            numeroTable = donne.numeroTable!!, numeroDonne = donne.numeroDonne,
                                            indexDonneJouee = donne.indexDonneJouee, contrat = donne.contrat,
                                            declarant = donne.declarant, resultatContrat = donne.signe,
                                            points = donne.points, nombrePlis = donne.plis, carteEntame = donne.carteEntame,
                                            historiqueJson = gson.toJson(donne.historique), mainsJson = gson.toJson(donne.mains)
                                        )
                                            ...
                                            declarant = donne.declarant, resultatContrat = donne.signe,
                                            points = donne.points, nombrePlis = donne.plis, carteEntame = donne.carteEntame,
                                            historique = donne.historique, mains = donne.mains
                                        )
                                    }
                                    Log.i("ClientActivity", "🔍 retour=$retour NBRE_DONNES=${TournoiConfig.NBRE_DONNES_PAR_TABLE} NBRE_MOUVEMENTS=${TournoiConfig.NBRE_MOUVEMENTS}")
                                    if (retour < 0) { afficherErreur("Résultat impossible à enregistrer.", EtapeClient.MOUVEMENT_EN_COURS); return@LaunchedEffect }
                                    if (retour >= TournoiConfig.NBRE_DONNES_PAR_TABLE) {
                                        ...
                                        if (mvntCourant.mvntNumero == TournoiConfig.NBRE_MOUVEMENTS) {
                                            ...
                                            equipesMvntTermine.value = mvntCourant.equipeNS to mvntCourant.equipeEO
                                            etape.value = EtapeClient.CHANGEMENT_DE_MOUVEMENT
                                        }
                                    } else {
                                        mouvement.value = mouvement.value?.copy(indexDonneAJouer = retour)
                                        contratFinal.value = null; aCliqueAfficherMains.value = false; mainsSelectionnees.value = null
                                        etape.value = EtapeClient.MOUVEMENT_EN_COURS
                                    }
                                }
                            }
                            ...
                            EtapeClient.CHANGEMENT_DE_MOUVEMENT -> {
                                ...
                                LaunchedEffect(mvntTermineNumero.value, equipesMvntTermine.value) {
                                    if (changementDeMouvement.value == null) {
                                        val mvntTermine = mvntTermineNumero.value ?: return@LaunchedEffect
                                        val eqNS = equipesMvntTermine.value?.first ?: return@LaunchedEffect
                                        val eqEO = equipesMvntTermine.value?.second ?: return@LaunchedEffect
                                        changementDeMouvement.value = withContext(Dispatchers.IO) {
                                            if (useLocal) DatabaseManager.getFuturMouvement(context, tournoiId, mvntTermine, eqNS, eqEO)
                                            else ClientNetworkUtils.getFuturMouvement(tournoiId, mvntTermine, eqNS, eqEO)
                                        }
                                    }
                                }
                                EcranChangementMouvement(
                                    ...
                                    onVerifierFinMouvement = {
                                        val mvnt = mvntTermineNumero.value ?: return@EcranChangementMouvement false
                                        if (useLocal) DatabaseManager.toutesEquipesOntTermineMouvement(context, tournoiId, mvnt)
                                        else ClientNetworkUtils.toutesEquipesOntTermineMouvement(tournoiId, mvnt)
                                    },
                                        ...
                                        coroutineScope.launch {
                                            try {
                                                val details = withContext(Dispatchers.IO) {
                                                    if (useLocal) DatabaseManager.getDonneResultatDetails(context, tournoiId)
                                                    else ClientNetworkUtils.recupererDetailsDonnes(tournoiId)
                                                }
                                                if (details != null) {
                                                    listeDetailsDonnes  = details
                                                    // Retour vers CHANGEMENT_DE_MOUVEMENT après consultation
                                                    etapeRetourDetails  = EtapeClient.CHANGEMENT_DE_MOUVEMENT
                                                    etape.value         = EtapeClient.AFFICHAGE_DETAILS_RESULTATS
                                                }
                                            } catch (e: Exception) {
                                                Log.e("ClientActivity", "❌ Erreur classement provisoire : ${e.message}")
                                                afficherErreur("Impossible de charger le classement provisoire.", EtapeClient.CHANGEMENT_DE_MOUVEMENT)
                                            }
                                        }
                                    },
                                    onOk = {
                            ...
                            EtapeClient.VERIFIER_TOURNOI -> {
                                ...
                                LaunchedEffect(idTournoiNonNull) {
                                    ...
                                    if (idTournoiNonNull == null) { etatVerifie = "ID manquant."; return@LaunchedEffect }
                                    try {
                                        enCours = true
                                        val etat = withContext(Dispatchers.IO) {
                                            if (useLocal) DatabaseManager.verifierEtatTournoi(context, idTournoiNonNull)
                                            else ClientNetworkUtils.verifierEtatTournoi(idTournoiNonNull)
                                        }
                                        etatVerifie = etat
                                        when (etat) {
                                            "TERMINE"     -> etape.value = EtapeClient.TOURNOI_TERMINE
                                            "NON_TERMINE" -> etape.value = etapeRetour ?: EtapeClient.ATTENTE_CLASSEMENT
                                            else          -> etape.value = EtapeClient.ATTENTE_CLASSEMENT
                                        }
                                    } catch (e: Exception) {
                                        Log.e("ClientActivity", "❌ Erreur vérification tournoi : ${e.message}")
                                        afficherErreur("Impossible de vérifier l'état du tournoi.", EtapeClient.ATTENTE_CLASSEMENT)
                                    }
                                    finally { enCours = false }
                                }
                                EcranMessage(titre = "Vérification", message = if (enCours) "En cours..." else "État : $etatVerifie")
                            }
                            ...
                            EtapeClient.ATTENTE_CLASSEMENT -> {
                                EcranMessage(
                                    ...
                                    onOk = { etapeRetourNonTermine.value = EtapeClient.ATTENTE_CLASSEMENT; etape.value = EtapeClient.VERIFIER_TOURNOI }
                                )
                            }

                            EtapeClient.TOURNOI_TERMINE -> {
                                var classementAffiche by remember { mutableStateOf<List<ClassementItem>?>(null) }
                                var erreur  by remember { mutableStateOf<String?>(null) }
                                var enCours by remember { mutableStateOf(false) }
                                val idTournoiNonNull = tournoi.value?.first
                                val scope = rememberCoroutineScope()
                                LaunchedEffect(idTournoiNonNull) {
                                    if (idTournoiNonNull == null) { erreur = "ID non disponible."; return@LaunchedEffect }
                                    try {
                                        enCours = true; erreur = null
                                        classementAffiche = withContext(Dispatchers.IO) {
                                            if (useLocal) {
                                                val lignes       = DatabaseManager.getClassementStocke(context, idTournoiNonNull)
                                                ...
                                                lignes.map { c ->
                                                    ...
                                                    val orientation = if (typeTournoi == "Mitchell" || typeTournoi == "MitchellGueridon") {
                                        ...
                                        if (classementAffiche == null) erreur = "Impossible de récupérer les résultats."
                                    } catch (e: Exception) { erreur = e.message ?: "Erreur" }
                                    finally { enCours = false }
                                }
                                when {
                                    erreur != null                -> EcranMessage(titre = "Classement", message = erreur!!)
                                    enCours                       -> EcranMessage(titre = "Classement...", message = "Veuillez patienter...")
                                    classementAffiche == null     -> EcranMessage(titre = "Classement", message = "Préparation...")
                                    classementAffiche!!.isEmpty() -> EcranMessage(titre = "Classement", message = "Aucun classement disponible.")
                                        ...
                                        onConsulterDetails = {
                                            scope.launch {
                                                try {
                                                    val details = withContext(Dispatchers.IO) {
                                                        if (useLocal) DatabaseManager.getDonneResultatDetails(context, idTournoiNonNull!!)
                                                        else ClientNetworkUtils.recupererDetailsDonnes(idTournoiNonNull!!)
                                                    }
                                                    if (details != null) {
                                                        listeDetailsDonnes = details
                                                        // Retour vers TOURNOI_TERMINE après consultation
                                                        etapeRetourDetails = EtapeClient.TOURNOI_TERMINE
                                                        etape.value        = EtapeClient.AFFICHAGE_DETAILS_RESULTATS
                                                    }
                                                } catch (e: Exception) { Log.e("ClientActivity", "Erreur détails", e) }
                                            }
                                        }
                                    )
                                }
