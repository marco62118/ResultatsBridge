...
import embrun.fr.tournoibridgeonline.common.model.ClassementItem
...
import embrun.fr.tournoibridgeonline.BridgeApplication
...
import embrun.fr.tournoibridgeonline.common.model.AuthAssociation
import embrun.fr.tournoibridgeonline.common.model.AuthJoueur
import embrun.fr.tournoibridgeonline.common.model.CreationTournoiResult
...
object ClientNetworkUtils {
    ...
    suspend fun inscrireJoueur(nom: String, prenom: String, email: String, mdp: String, code: String): String {
        return try {
            val urlStr = "${embrun.fr.tournoibridgeonline.common.URL_SERVEUR_ONLINE}/traitement_inscription_joueur.php"
            ...
            org.json.JSONObject(response).optString("etat", "ERREUR")
        } catch (e: Exception) {
    ...
    suspend fun verifierTokenJoueur(token: String): Boolean {
        return try {
            val urlStr = "${embrun.fr.tournoibridgeonline.common.URL_SERVEUR_ONLINE}" +
            ...
            val valide = org.json.JSONObject(response).optString("etat") == "OK"
    ...
    suspend fun importerJoueursDepuisServeurEnLigne(token: String): List<Joueur> {
        return try {
            val urlStr = "${embrun.fr.tournoibridgeonline.common.URL_SERVEUR_ONLINE}/api_adherent.php?action=joueurs&token=$token"
            ...
            (0 until arr.length()).map { i ->
                ...
                Joueur(idJoueur = obj.getInt("ID"), nom = obj.getString("nom"), prenom = obj.getString("prenom"))
            }
        } catch (e: Exception) {
    ...
    suspend fun getFuturMouvement(
        ...
        return try {
            ...
            for (i in 0 until entriesArray.length()) {
                ...
                entriesList.add(ChangementDeMouvementEntry(
