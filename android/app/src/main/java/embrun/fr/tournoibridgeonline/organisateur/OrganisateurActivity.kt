...
import embrun.fr.tournoibridgeonline.common.utils.getSecurePrefs
...
import embrun.fr.tournoibridgeonline.organisateur.data.DatabaseHelper
import embrun.fr.tournoibridgeonline.organisateur.server.ServerManager
import embrun.fr.tournoibridgeonline.common.ClientConfig
import embrun.fr.tournoibridgeonline.client.ClientActivity
import embrun.fr.tournoibridgeonline.client.ClientNetworkUtils
import embrun.fr.tournoibridgeonline.common.URL_SERVEUR_ONLINE
import embrun.fr.tournoibridgeonline.common.model.ErreurLogItem
...
class OrganisateurActivity : BaseActivity() {
    ...
    private var organisateurResteOrganisateur = false  // ← AJOUTER
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        creationLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == RESULT_OK) {
                ...
                if (idTournoi != -1) {
                    idTournoiState.value = idTournoi; typeTournoiState.value = typeTournoi; tournoiOuvertState.value = true
            ...
            if (result.resultCode == RESULT_OK) {
                ...
                if (equipesSuccess && idTournoi != -1) {
                    ...
                    val intent = Intent(this, ParticipationOrganisateurActivity::class.java).apply {
                        putExtra("id_tournoi", idTournoi); putExtra("ip_serveur", ipAUtiliser)
                    }
                                ...
                                if (journalErreurs.isEmpty()) {
                                    ...
                                    Column(
                                        ...
                                        journalErreurs.forEach { log ->
                                            val equipeText = if (log.equipeNumero > 0) "Éq. ${log.equipeNumero}" else "—"
                                            Card(
                    ...
                    Column(modifier = Modifier.fillMaxWidth().padding(16.dp)) {
                        ...
                        if (!modeOnlineState.value) {
                            ...
                            if (tournoiOuvertState.value) {
                                ...
                                Button(
                                    onClick = {
                                        val success = DatabaseManager.fermerTournoiOuvert(this@OrganisateurActivity)
                                        if (success) {
                                            idTournoiState.value = null; typeTournoiState.value = ""
                                            tournoiOuvertState.value = false; ipServeurState.value = ""
                                            Toast.makeText(this@OrganisateurActivity, "Tournoi fermé", Toast.LENGTH_SHORT).show()
                                        }
                                    },
                                    modifier = Modifier.fillMaxWidth()
                                ) { Text("🧹 Fermer les tournois ouverts") }
                                Spacer(Modifier.height(16.dp))
                            }
                            ...
                            var emailExport       by remember { mutableStateOf(securePrefs.getString("lastEmail", "") ?: "") }
                            var mdpExport         by remember { mutableStateOf(securePrefs.getString("lastMdp", "") ?: "") }
                            ...
                            if (showResultatExport && exportMessage != null) {
                                AlertDialog(
                                    ...
                                    text  = { Text(exportMessage!!, style = MaterialTheme.typography.bodyLarge) },
                                    confirmButton = { Button(onClick = { showResultatExport = false }) { Text("OK") } }
                                )
                            }
                            ...
                            if (showDialogExport) {
                                AlertDialog(
                                    ...
                                    text = {
                                        Column {
                                            Text("Entrez vos identifiants TournoiBridgeOnline :", style = MaterialTheme.typography.bodyMedium)
                                            Spacer(Modifier.height(8.dp))
                                            OutlinedTextField(
                                                value = emailExport, onValueChange = { emailExport = it },
                                            ...
                                            OutlinedTextField(
                                                ...
                                                trailingIcon = {
                                                    IconButton(onClick = { mdpExportVisible = !mdpExportVisible }) {
                                                        Icon(
                                    ...
                                    confirmButton = {
                                        Button(onClick = {
                                            ...
                                            scope.launch {
                                                ...
                                                if (idTournoi == null) {
                                                    exportMessage = "❌ Aucun tournoi terminé trouvé."
                                                ...
                                                lancerExport(
                                                    ...
                                                    onMessage   = { msg -> exportMessage = msg; showResultatExport = true },
                                                    onSuccess   = {
                                    ...
                                    dismissButton = { OutlinedButton(onClick = { showDialogExport = false }) { Text("Annuler") } }
                                )
                            }
                                ...
                                Button(
                                    onClick = {
                                        scope.launch {
                                            ...
                                            val emailMemo = securePrefs.getString("lastEmail", "") ?: ""
                                    ...
                                    Text(if (exportEnCours) "⏳ Export en cours..." else "☁️ Exporter le tournoi")
                                }
                                Spacer(Modifier.height(16.dp))
                            }
                            ...
                            Button(
                                onClick = {
                                    ...
                                    if (ip == null) {
                                        Toast.makeText(this@OrganisateurActivity, "Impossible de démarrer le serveur", Toast.LENGTH_LONG).show()
                                    } else {
                                        ipServeurState.value = ip
                                        val tournoi = DatabaseManager.getTournoiOuvert(this@OrganisateurActivity)
                                        if (tournoi != null) {
                                            idTournoiState.value = tournoi.first; typeTournoiState.value = tournoi.second; tournoiOuvertState.value = true
                                            val intent = Intent(this@OrganisateurActivity, ParticipationOrganisateurActivity::class.java).apply {
                                                putExtra("id_tournoi", tournoi.first); putExtra("ip_serveur", ip)
                                            }
                                            participationLauncher.launch(intent)
                                        } else {
                                            val intent = Intent(this@OrganisateurActivity, CreationTournoiActivity::class.java).apply {
                                                putExtra("ip_serveur", ip); putExtra("mode_online", false)
                                            }
                                ...
                                modifier = Modifier.fillMaxWidth()
                            ) { Text(if (tournoiOuvertState.value) "Démarrer le serveur" else "Créer un nouveau tournoi") }

                            if (ipServeurState.value.isNotEmpty()) {
                                Spacer(Modifier.height(8.dp))
                                Text("Serveur : ${ipServeurState.value}", style = MaterialTheme.typography.bodyLarge)
                            }
                        ...
                        if (modeOnlineState.value) {
                            ...
                            if (!estConnecteOnline) {
                                ...
                                Button(
                                    onClick = {
                                        if (emailInput.isBlank() || mdpInput.isBlank()) { messageErreur = "Email et mot de passe obligatoires"; return@Button }
                                        scope.launch {
                                            ...
                                            val auth = withContext(Dispatchers.IO) {
                                                ...
                                                ClientNetworkUtils.connexionAssociation(emailInput, mdpInput)
                                            }
                                            if (auth == null) { messageErreur = "❌ Email ou mot de passe incorrect"; chargement = false; return@launch }
                                            securePrefs.edit().putString("lastEmail", emailInput).putString("lastMdp", mdpInput).apply()
                                            nomAssociation = auth.nom
                                            val codeExistant = withContext(Dispatchers.IO) {
                                                try {
                                                    val url = "${URL_SERVEUR_ONLINE}/serverBridge.php/getCodeAdherent?token_api=${auth.tokenApi}"
                                                    val resp = java.net.URL(url).readText()
                                                    org.json.JSONObject(resp).optString("code_adherent", "")
                                                } catch (e: Exception) { "" }
                                            }
                                            codeAdherent = codeExistant; nouveauCode = codeExistant
                                            val tournoi = withContext(Dispatchers.IO) { ClientNetworkUtils.verifierTournoiOuvert() }
                                            estConnecteOnline = true; chargement = false
                                            if (tournoi != null) { idTournoiState.value = tournoi.first; typeTournoiState.value = tournoi.second; tournoiOuvertState.value = true }
                                            else tournoiOuvertState.value = false
                                        }
                                    },
                                    modifier = Modifier.fillMaxWidth(), enabled = !chargement
                            ...
                            if (estConnecteOnline) {
                                Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.tertiaryContainer), modifier = Modifier.fillMaxWidth()) {
                                    Column(modifier = Modifier.padding(12.dp)) {
                                        Text("✅ Connecté", style = MaterialTheme.typography.titleSmall)
                                        Text(nomAssociation, style = MaterialTheme.typography.bodyMedium)
                                    }
                                }
                                Spacer(Modifier.height(8.dp))
                                ...
                                Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.secondaryContainer), modifier = Modifier.fillMaxWidth()) {
                                    Column(modifier = Modifier.padding(12.dp)) {
                                        Text("🔑 Code adhérent", style = MaterialTheme.typography.titleSmall)
                                        Spacer(Modifier.height(4.dp))
                                        if (codeAdherent.isNotEmpty()) {
                                            Text(codeAdherent, style = MaterialTheme.typography.headlineSmall, color = MaterialTheme.colorScheme.primary)
                                            Text("Donnez ce code à vos adhérents pour qu'ils s'inscrivent", style = MaterialTheme.typography.bodySmall)
                                        } else Text("Aucun code défini", style = MaterialTheme.typography.bodySmall)
                                        Spacer(Modifier.height(8.dp))
                                        OutlinedButton(onClick = { editCodeVisible = !editCodeVisible }, modifier = Modifier.fillMaxWidth()) {
                                            Text(if (editCodeVisible) "Annuler" else "✏️ Modifier le code")
                                        }
                                        if (editCodeVisible) {
                                            Spacer(Modifier.height(8.dp))
                                            OutlinedTextField(value = nouveauCode, onValueChange = { nouveauCode = it.uppercase() },
                                                label = { Text("Nouveau code") }, placeholder = { Text("Ex: EMBRUN25") },
                                                modifier = Modifier.fillMaxWidth(), singleLine = true)
                                            msgCode?.let {
                                                Spacer(Modifier.height(4.dp))
                                            ...
                                            Spacer(Modifier.height(8.dp))
                                            Button(onClick = {
                                                if (nouveauCode.length < 4) { msgCode = "❌ Code trop court (4 caractères minimum)"; return@Button }
                                                scope.launch {
                                                    val result = withContext(Dispatchers.IO) { ClientNetworkUtils.majCodeAdherent(nouveauCode) }
                                                    when (result) {
                                                        "OK"          -> { codeAdherent = nouveauCode; msgCode = "✅ Code mis à jour"; editCodeVisible = false }
                                                        "CODE_EXISTE" -> msgCode = "❌ Ce code est déjà utilisé par un autre club"
                                ...
                                if (tournoiOuvertState.value) {
                                    Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.secondaryContainer),
                                        ...
                                        Column(modifier = Modifier.padding(12.dp)) {
                                            Text("🎯 Tournoi en cours", style = MaterialTheme.typography.titleSmall)
                                            Spacer(Modifier.height(4.dp))
                                            Text("ID : ${idTournoiState.value}", style = MaterialTheme.typography.bodyMedium)
                                            Text("Type : ${typeTournoiState.value}", style = MaterialTheme.typography.bodyMedium)
                                        }
                                    }
                                    Spacer(Modifier.height(8.dp))
                                    Button(onClick = {
                                        val intent = Intent(this@OrganisateurActivity, ParticipationOrganisateurActivity::class.java).apply {
                                            putExtra("id_tournoi", idTournoiState.value ?: -1); putExtra("ip_serveur", URL_SERVEUR_ONLINE)
                                        }
                                        participationLauncher.launch(intent)
                                    }, modifier = Modifier.fillMaxWidth()) { Text("▶️ Reprendre le tournoi") }
                                    Spacer(Modifier.height(8.dp))
                                    Button(onClick = {
                                        scope.launch {
                                            val success = withContext(Dispatchers.IO) { ClientNetworkUtils.fermerTournoiOuvert() }
                                            if (success) { idTournoiState.value = null; typeTournoiState.value = ""; tournoiOuvertState.value = false; Toast.makeText(this@OrganisateurActivity, "Tournoi fermé", Toast.LENGTH_SHORT).show() }
                                            else Toast.makeText(this@OrganisateurActivity, "Erreur fermeture tournoi", Toast.LENGTH_LONG).show()
                                        }
                                    }, modifier = Modifier.fillMaxWidth()) { Text("🧹 Fermer le tournoi Online") }
                                }

                                if (!tournoiOuvertState.value) {
                                    Button(onClick = {
                                        val intent = Intent(this@OrganisateurActivity, CreationTournoiActivity::class.java).apply {
                                            putExtra("ip_serveur", URL_SERVEUR_ONLINE); putExtra("mode_online", true)
                                        }
                                        creationLauncher.launch(intent)
                                    }, modifier = Modifier.fillMaxWidth()) { Text("🌐 Créer un tournoi Online") }
                                }
                                ...
                                if (idTournoiPourLog != null) {
                                    ...
                                    OutlinedButton(
                                        onClick = {
                                            scope.launch {
                                                journalErreurs = withContext(Dispatchers.IO) {
                                                    ClientNetworkUtils.recupererErreursLog(idTournoiPourLog)
                                                }
                                                showJournalErreurs = true
                                            }
                                        },
    ...
    override fun onResume() {
        ...
        if (organisateurResteOrganisateur) { organisateurResteOrganisateur = false; return } // ← stoppe la boucle
        val ipAUtiliser = if (modeOnlineState.value) URL_SERVEUR_ONLINE else ipServeurState.value
