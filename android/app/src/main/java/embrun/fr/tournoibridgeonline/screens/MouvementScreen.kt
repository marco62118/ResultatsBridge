...
import embrun.fr.tournoibridgeonline.common.model.ContratInfo
import embrun.fr.tournoibridgeonline.common.model.DonneDataAEnregistrer
import embrun.fr.tournoibridgeonline.common.model.Equipe
import embrun.fr.tournoibridgeonline.common.model.Mouvement
import embrun.fr.tournoibridgeonline.common.model.ResultatInfo
import embrun.fr.tournoibridgeonline.common.model.Tour
import embrun.fr.tournoibridgeonline.common.model.TournoiConfig
import embrun.fr.tournoibridgeonline.common.ui.components.DropdownMenuCouleur
import embrun.fr.tournoibridgeonline.common.ui.components.DropdownMenuDeclarant
import embrun.fr.tournoibridgeonline.common.ui.components.DropdownMenuEntameCarte
import embrun.fr.tournoibridgeonline.common.ui.components.DropdownMenuEntameCouleur
import embrun.fr.tournoibridgeonline.common.ui.components.DropdownMenuInsulte
import embrun.fr.tournoibridgeonline.common.ui.components.DropdownMenuNiveau
import embrun.fr.tournoibridgeonline.common.ui.components.DropdownMenuPlis
import embrun.fr.tournoibridgeonline.common.ui.components.DropdownMenuSigne
import embrun.fr.tournoibridgeonline.common.utils.calculerPoints
...
fun MouvementScreen(
    ...
    val contratVerrouille         = false
    val entameOuResultatVerrouille = false
    ...
    val executerEnvoi = {
        ...
        onEnregistrerDonne(DonneDataAEnregistrer(
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(14.dp)) {
                        Column(modifier = Modifier.width(52.dp)) {
                            ...
                            Box(modifier = Modifier.width(52.dp).height(44.dp)) {
                                DropdownMenuNiveau(selected = contratState.value.niveau, onSelect = { contratState.value = contratState.value.copy(niveau = it) }, isEnabled = true, backgroundColor = fondActif)
                            }
                        }
                        Column(modifier = Modifier.width(58.dp)) {
                            ...
                            Box(modifier = Modifier.width(58.dp).height(44.dp)) {
                                DropdownMenuCouleur(selected = contratState.value.couleur, onSelect = { contratState.value = contratState.value.copy(couleur = it) }, isEnabled = true, backgroundColor = fondActif)
                            }
                        }
                        Column(modifier = Modifier.width(58.dp)) {
                            Text("Insulte", style = MaterialTheme.typography.labelSmall)
                            Box(modifier = Modifier.width(58.dp).height(44.dp)) {
                                DropdownMenuInsulte(selected = contratState.value.insulte, onSelect = { contratState.value = contratState.value.copy(insulte = it) }, isEnabled = true, backgroundColor = fondActif)
                            }
                        }
                        Column(modifier = Modifier.width(74.dp)) {
                            ...
                            Box(modifier = Modifier.width(74.dp).height(44.dp)) {
                                DropdownMenuDeclarant(selected = contratState.value.declarant, onSelect = { contratState.value = contratState.value.copy(declarant = it) }, isEnabled = true, backgroundColor = fondActif)
                            }
                        }
                    }
                }
                    ...
                    Row(modifier = Modifier.fillMaxWidth().padding(vertical = 6.dp), horizontalArrangement = Arrangement.Center, verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Column(modifier = Modifier.padding(horizontal = 8.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                            ...
                            Box(modifier = Modifier.width(66.dp).height(48.dp)) {
                                DropdownMenuEntameCarte(selected = contratState.value.entameCarte, onSelect = { aCliqueSurEntameCarte.value = true; contratState.value = contratState.value.copy(entameCarte = it) }, isEnabled = true, backgroundColor = fondActif)
                            }
                        }
                        Column(modifier = Modifier.padding(horizontal = 8.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                            Text("Couleur", style = MaterialTheme.typography.labelSmall)
                            Box(modifier = Modifier.width(66.dp).height(48.dp)) {
                                DropdownMenuEntameCouleur(selected = contratState.value.entameCouleur, onSelect = { aCliqueSurEntameCouleur.value = true; contratState.value = contratState.value.copy(entameCouleur = it) }, isEnabled = true, backgroundColor = fondActif)
                            }
                        }
                    }
                    ...
                    if (entameComplete) {
                        ...
                        if (!aCliqueAfficherResultats.value) {
                            ...
                            Box(modifier = Modifier.fillMaxWidth(), contentAlignment = Alignment.Center) {
                                Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                                    Box(modifier = Modifier.width(84.dp).height(44.dp)) {
                                        DropdownMenuSigne(selected = resultat.value.signe, plis = resultat.value.plis, onSelect = { aCliqueSurSigne.value = true; resultat.value = resultat.value.copy(signe = it) }, isEnabled = true, backgroundColor = fondActif)
                                    }
                                    Box(modifier = Modifier.width(84.dp).height(44.dp)) {
                                        DropdownMenuPlis(selected = resultat.value.plis, niveauContrat = contratState.value.niveau, signe = resultat.value.signe, onSelect = { aCliqueSurPlis.value = true; resultat.value = resultat.value.copy(plis = it) }, isEnabled = true, backgroundColor = fondActif)
                                    }
                            ...
                            LaunchedEffect(
                                ...
                                ) calculerPoints(insulte, couleur, vulnerable, declarant, niveau, signe, plis)
                                else 9999
