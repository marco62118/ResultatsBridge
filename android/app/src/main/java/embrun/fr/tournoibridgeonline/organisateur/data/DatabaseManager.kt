package embrun.fr.tournoibridgeonline.organisateur.data
import android.content.ContentValues
import android.content.Context
import android.database.sqlite.SQLiteDatabase
import android.util.Log
import embrun.fr.tournoibridgeonline.common.model.ClassementManager
import embrun.fr.tournoibridgeonline.common.model.ClassementManager4Equ2T21D
import embrun.fr.tournoibridgeonline.common.model.Equipe
import embrun.fr.tournoibridgeonline.common.model.Joueur
import embrun.fr.tournoibridgeonline.common.model.Mouvement
import embrun.fr.tournoibridgeonline.common.model.MouvementResult
import embrun.fr.tournoibridgeonline.common.model.DonneDetail
...
import embrun.fr.tournoibridgeonline.common.model.DonneResultatDetail
import embrun.fr.tournoibridgeonline.common.model.AnnonceJoueur
import embrun.fr.tournoibridgeonline.common.model.ChangementDeMouvement
import embrun.fr.tournoibridgeonline.common.model.ChangementDeMouvementEntry
import embrun.fr.tournoibridgeonline.common.model.DonneComplete
import embrun.fr.tournoibridgeonline.common.model.EquipeDonneInfo
        ...
        return try {
            db.rawQuery(
                ...
                if (cursor.moveToFirst()) {
                    ...
                    val nbreEquipes       = cursor.getInt(cursor.getColumnIndexOrThrow("nbre_equipe"))
    ...
    private fun getMouvementMitchellGueridon(
        ...
        if (!estRelais) {
            for (numDonne in premiereDonne..derniereDonne) {
                ...
                listeDonnes.add(DonneDetail(numero = numDonne, donneur = donneur, vulnerable = vulnerable, mains = mainsListe))
            }
    ...
    fun getTousLesJoueurs(context: Context): List<Joueur> {
        ...
        return try {
            db.rawQuery("SELECT ID, nom, prenom FROM joueurs ORDER BY ID ASC", null).use { cursor ->
                while (cursor.moveToNext()) {
                    joueurs.add(Joueur(idJoueur = cursor.getInt(0), nom = cursor.getString(1), prenom = cursor.getString(2)))
                }
            }
        ...
        return try {
            db.rawQuery(
                ...
                while (cursor.moveToNext()) {
                    ...
                    val joueur1 = Joueur(cursor.getInt(cursor.getColumnIndexOrThrow("id_joueur1")), cursor.getString(cursor.getColumnIndexOrThrow("joueur1_nom")), cursor.getString(cursor.getColumnIndexOrThrow("joueur1_prenom")))
                    val joueur2 = Joueur(cursor.getInt(cursor.getColumnIndexOrThrow("id_joueur2")), cursor.getString(cursor.getColumnIndexOrThrow("joueur2_nom")), cursor.getString(cursor.getColumnIndexOrThrow("joueur2_prenom")))
                ...
                return when (typeDeTournoi) {
                    ...
                    db.rawQuery(sqlMvnt, arrayOf(idTournoi.toString(), idTournoi.toString(), equipeNumero.toString(), equipeNumero.toString(), mvntNumero.toString())).use { cursorMv ->
                        ...
                        for (i in 1..nbreDonnesParTable) {
                            ...
                            val mainsListe = mainsMap?.let { map -> listOf("N", "E", "S", "O").map { seat -> map[seat] ?: emptyList() } }
                            listeDonnes.add(DonneDetail(numero = donneIdInDV, donneur = metaData.first, vulnerable = metaData.second, mains = mainsListe))
                        }
                        val mvnt = Mouvement(
    ...
    fun getResultatsTournoi(context: Context, idTournoi: Int): List<ClassementManager.ResultatLigne> {
        ...
        try {
            db.rawQuery("SELECT numero_donne, numero_table, equipeNS, equipeEO, pointsNS, pointsEO FROM resultats WHERE id_tournoi = ? ORDER BY numero_donne ASC, numero_table ASC", arrayOf(idTournoi.toString())).use { cursor ->
                while (cursor.moveToNext()) {
                    resultats.add(ClassementManager.ResultatLigne(
    ...
    fun getDonneResultatDetails(context: Context, idTournoi: Int): List<DonneResultatDetail> {
        ...
        return try {
            db.rawQuery(query, arrayOf(idTournoi.toString())).use { cursor ->
                if (cursor.moveToFirst()) {
                    do {
                        list.add(DonneResultatDetail(
                            numeroDonne = cursor.getInt(0), equipeNS = cursor.getInt(1), equipeEO = cursor.getInt(2),
                            contrat = cursor.getString(3), declarant = cursor.getString(4), resultatContrat = cursor.getString(5),
                            nombrePli = cursor.getInt(6), carteEntame = cursor.getString(7),
                            pointsNS = cursor.getInt(8), pointsEO = cursor.getInt(9),
                            ptsNS = cursor.getDouble(10), ptsEO = cursor.getDouble(11),
        ...
        try {
            ...
            if (typeTournoi == "Mitchell") {
                ...
                for (tableNumero in 1..nbreTables) {
                    ...
                    var ns = Equipe(equipeNumero = nsNum,
                        joueur1 = Joueur(0, "", ""), joueur2 = Joueur(0, "", ""), idTournoi = idTournoi)
                    db.rawQuery(
                       ...
                       WHERE e.id_tournoi = ? AND e.equipe_numero = ?""",
                        ...
                        if (c.moveToFirst()) ns = Equipe(equipeNumero = nsNum,
                    ...
                    var eo = Equipe(equipeNumero = eoNum,
                        joueur1 = Joueur(0, "", ""), joueur2 = Joueur(0, "", ""), idTournoi = idTournoi)
                    db.rawQuery(
                        """SELECT j1.ID, j1.nom, j1.prenom, j2.ID, j2.nom, j2.prenom
                       FROM equipes e
                       ...
                       WHERE e.id_tournoi = ? AND e.equipe_numero = ?""",
                        ...
                        if (c.moveToFirst()) eo = Equipe(equipeNumero = eoNum,
                            joueur1 = Joueur(c.getInt(0), c.getString(1), c.getString(2)),
                            joueur2 = Joueur(c.getInt(3), c.getString(4), c.getString(5)),
                            idTournoi = idTournoi)
                    }

                    entries.add(ChangementDeMouvementEntry(tableNumero = tableNumero, equipe = ns, adversaire = eo))
                ...
                return if (entries.isNotEmpty()) ChangementDeMouvement(mvntSuivant = mvntSuivant, entries = entries)
        ...
        WHERE eNS.id_tournoi = ? AND eEO.id_tournoi = ? AND t.mvnt_numero = ?
          AND ((t.equipe_NS IN (?, ?)) OR (t.equipe_EO IN (?, ?)))""".trimIndent()
            ...
            db.rawQuery(sql, arrayOf(idTournoi.toString(), idTournoi.toString(), mvntSuivant.toString(), equipeNS.toString(), equipeEO.toString(), equipeNS.toString(), equipeEO.toString())).use { cursor ->
                while (cursor.moveToNext()) {
                    val equipeNSObj = Equipe(equipeNumero = cursor.getInt(cursor.getColumnIndexOrThrow("equipe_NS")),
                        joueur1 = Joueur(cursor.getInt(cursor.getColumnIndexOrThrow("jns1_id")), cursor.getString(cursor.getColumnIndexOrThrow("jns1_nom")), cursor.getString(cursor.getColumnIndexOrThrow("jns1_prenom"))),
                        joueur2 = Joueur(cursor.getInt(cursor.getColumnIndexOrThrow("jns2_id")), cursor.getString(cursor.getColumnIndexOrThrow("jns2_nom")), cursor.getString(cursor.getColumnIndexOrThrow("jns2_prenom"))),
                        idTournoi = idTournoi)
                    val equipeEOObj = Equipe(equipeNumero = cursor.getInt(cursor.getColumnIndexOrThrow("equipe_EO")),
                        joueur1 = Joueur(cursor.getInt(cursor.getColumnIndexOrThrow("jeo1_id")), cursor.getString(cursor.getColumnIndexOrThrow("jeo1_nom")), cursor.getString(cursor.getColumnIndexOrThrow("jeo1_prenom"))),
                        joueur2 = Joueur(cursor.getInt(cursor.getColumnIndexOrThrow("jeo2_id")), cursor.getString(cursor.getColumnIndexOrThrow("jeo2_nom")), cursor.getString(cursor.getColumnIndexOrThrow("jeo2_prenom"))),
                        idTournoi = idTournoi)
                    entries.add(ChangementDeMouvementEntry(tableNumero = cursor.getInt(cursor.getColumnIndexOrThrow("table_numero")), equipe = equipeNSObj, adversaire = equipeEOObj))
                }
            }
    ...
    fun getDonneComplete(context: Context, idTournoi: Int, numeroDonne: Int, equipeNS: Int): DonneComplete? {
        ...
        return try {
            ...
            db.rawQuery("SELECT joueur, annonce FROM encheres WHERE id_tournoi = ? AND numero_donne = ? AND equipeNS = ? ORDER BY ordre ASC",
                arrayOf(idTournoi.toString(), numeroDonne.toString(), equipeNS.toString())).use { c -> while (c.moveToNext()) encheres.add(AnnonceJoueur(c.getString(0), c.getString(1))) }
            if (encheres.isEmpty()) {
                db.rawQuery("SELECT joueur, annonce FROM encheres WHERE id_tournoi = ? AND numero_donne = ? ORDER BY ordre ASC",
                    arrayOf(idTournoi.toString(), numeroDonne.toString())).use { c -> while (c.moveToNext()) encheres.add(AnnonceJoueur(c.getString(0), c.getString(1))) }
            }
    ...
    fun getEquipesAyantJoueDonne(context: Context, idTournoi: Int, numeroDonne: Int): List<EquipeDonneInfo> {
        ...
        return try {
            db.rawQuery("SELECT equipeNS, equipeEO, contrat, declarant FROM resultats WHERE id_tournoi = ? AND numero_donne = ? ORDER BY equipeNS ASC",
                ...
                while (cursor.moveToNext()) { liste.add(EquipeDonneInfo(equipeNS = cursor.getInt(0), equipeEO = cursor.getInt(1), contrat = cursor.getString(2) ?: "", declarant = cursor.getString(3) ?: "")) }
            }
