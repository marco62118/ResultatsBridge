<?php
	...
	// =========================================================
	// 🔐 Récupération du token et de l'id_association
	// Accepte token_api (organisateur) ou token_joueur (adhérent/externe)
	// =========================================================
	$tokenApi    = $_GET['token_api']    ?? $_POST['token_api']    ?? ($_SERVER['HTTP_X_TOKEN_API'] ?? '');
	$tokenJoueur = $_GET['token_joueur'] ?? $_POST['token_joueur'] ?? '';

	$idAssociation = 0;
	if (!empty($tokenApi)) {
		$idAssociation = DatabaseManager::getIdAssociation($tokenApi);
		logServer("🔑 token_api=" . substr($tokenApi, 0, 8) . "... → id_association=$idAssociation");
	} elseif (!empty($tokenJoueur)) {
		$joueur = DatabaseManager::getAdherentParToken($tokenJoueur);
		$idAssociation = $joueur ? $joueur['idAssociation'] : 0;
		logServer("🔑 token_joueur=" . substr($tokenJoueur, 0, 8) . "... → id_association=$idAssociation");
	}
	...
	case str_contains($path, '/verifierTournoiOuvert'):
		...
		if ($tournoi) {
			...
			$data = [
				...
				'equipeRelais'      => (int)$equipeRelais,
				'nbreTables'        => (int)$tournoi['nbre_tables']
			];
			logServer("✅ Tournoi ouvert : ID=" . $data['id']);
			echo json_encode($data);
		} else {
			echo json_encode(['id' => 0, 'type' => '', 'nbreMouvements' => 0, 'nbreDonnesParTable' => 0, 'equipeRelais' => 0, 'nbreTables' => 0]);
			logServer("⚠️ Aucun tournoi ouvert");
		}
		break;
...
case str_contains($path, '/creerTournoi'):
	...
	// Mitchell / Guéridon : nbreEquipes = 0 autorisé (inconnu à la création)
	// Howell   : nbreEquipes doit être > 0
	if (empty($type) || ($type !== 'Mitchell' && $type !== 'MitchellGueridon' && $nbreEquipes <= 0)) {
	...
	// =========================================================
	// 🏆 Mise à jour du tournoi Mitchell après constitution des équipes
	// =========================================================
	case str_contains($path, '/miseAJourTournoiMitchell'):
		logServer("▶️ Action reçue : miseAJourTournoiMitchell");
		$data              = json_decode(file_get_contents("php://input"), true);
		$idTournoi         = intval($data['idTournoi']         ?? 0);
		$nbreEquipes       = intval($data['nbreEquipes']       ?? 0);
		$nbreDonnes        = intval($data['nbreDonnes']        ?? 0);
		$nbreEnregistrement = intval($data['nbreEnregistrement'] ?? 0);
		$nbreMouvements    = intval($data['nbreMouvements']    ?? 0);

		if ($idTournoi <= 0 || $nbreEquipes <= 0) {
			echo json_encode(["etat" => "ERREUR_PARAM"], JSON_UNESCAPED_UNICODE);
			break;
		}

		$nbreDonnesParTable = ($nbreMouvements > 0) ? intdiv($nbreDonnes, $nbreMouvements) : 0;
		$ok = DatabaseManager::miseAJourTournoiMitchell(
			$idTournoi, $nbreEquipes, $nbreDonnesParTable, $nbreEnregistrement, $nbreMouvements
		);
		echo json_encode(["etat" => $ok ? "OK" : "ERREUR"], JSON_UNESCAPED_UNICODE);
		break;	
	...
	// =========================================================
	// 📋 Positions des équipes au mouvement 1 (NS/EO + table)
	// =========================================================
	case str_contains($path, '/getPositionsMouvement1'):
		logServer("▶️ Action reçue : getPositionsMouvement1");
		// Le type de tournoi n'est plus accepté du client : il est lu en BD
		// dans DatabaseManager::getPositionsMouvement1() à partir de l'idTournoi.
		...
		$positions = DatabaseManager::getPositionsMouvement1($idTournoi);
		echo json_encode([
			"etat"      => "OK",
			"positions" => $positions
		], JSON_UNESCAPED_UNICODE);
	...
	if ($resultat->status === "MouvementComplet") {
    ...
    $response = json_encode(["etat" => "ERREUR_INCONNUE"], JSON_UNESCAPED_UNICODE);
}
 ...
 */
	 ...
	 */
	 ...
	 case str_contains($path, '/getResultatsTournoi'):
		...
		$tokenPublic = DatabaseManager::genererOuGetTokenPublic($idTournoi);
		logServer("✅ Classement calculé et enregistré pour le tournoi $idTournoi (type=$typeTournoi) tokenPublic=" . substr($tokenPublic, 0, 8));
	...
	case str_contains($path, '/importerTournoi'):
		...
		if ($result['etat'] === 'OK') {
			logServer("✅ Tournoi importé → idTournoiCloud=" . $result['idTournoi'] . " tokenPublic=" . substr($result['tokenPublic'] ?? '', 0, 8));
			echo json_encode(["etat" => "OK", "idTournoi" => $result['idTournoi'], "tokenPublic" => $result['tokenPublic'] ?? ''], JSON_UNESCAPED_UNICODE);
		} else {
	...
	// =========================================================
	// 📋 Log d'erreur joueur (appelé par le client sans token)
	// =========================================================
	case str_contains($path, '/logErreur'):
		logServer("▶️ Action reçue : logErreur");
		$idTournoi    = intval($query['idTournoi']    ?? -1);
		$equipeNumero = intval($query['equipeNumero'] ?? -1);
		$etape        = trim($_GET['etape']   ?? '');
		$message      = trim($_GET['message'] ?? '');
		DatabaseManager::enregistrerErreurLog($idTournoi, $equipeNumero, $etape, $message);
		echo json_encode(["etat" => "OK"], JSON_UNESCAPED_UNICODE);
		break;

	// =========================================================
	// 📋 Lecture journal erreurs (organisateur avec idTournoi)
	// =========================================================
	case str_contains($path, '/getErreursLog'):
		logServer("▶️ Action reçue : getErreursLog");
		$idTournoi = intval($query['idTournoi'] ?? 0);
		if ($idTournoi <= 0) {
			echo json_encode([], JSON_UNESCAPED_UNICODE);
			break;
		}
		$logs = DatabaseManager::getErreursLog($idTournoi);
